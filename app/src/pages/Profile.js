    ...
    const viewCart = () => {
        if(Array.isArray(cart) && cart.length){
            return (
                <div>
                    {cart.map((item, index) => (
                        item && <CartItem key={index} cart={cart} item={item} onAdd={onAdd} onRemove={onRemove} />
                    ))}
                </div>
            );
        }
            ...
            return (
                <div>
                    {wishlist.map((item, index) => (
                        <WishItem key={index} item={item} onTapRemove={removeFromWishlist} />
                    ))}
                </div>
            );
        }
            ...
            return (
                <div>
                    {orders.map((item, index) => (
                        <OrderItem key={index} item={item} onTapViewMore={() => {}} />
                    ))}
                </div>
            );
        }
                    ...
                    <div className="col-12">
                        <nav>
                            <div className="nav nav-tabs" id="nav-tab" role="tablist">
                                <a className="nav-link active" style={{ color: '#3680B4'}} id="nav-cart-tab" data-toggle="tab" href="#nav-cart" role="tab" aria-controls="nav-cart" aria-selected="true">
                                    <i className="fas fa-shopping-cart mr-3"></i>Cart
                                </a>
                                <a className="nav-link" style={{ color: '#3680B4'}} id="nav-wishlist-tab" data-toggle="tab" href="#nav-wishlist" role="tab" aria-controls="nav-wishlist" aria-selected="false">
                                    <i className="fas fa-heart mr-3"></i>Wishlist
                                </a>
                                <a className="nav-link" style={{ color: '#3680B4'}} id="nav-orders-tab" data-toggle="tab" href="#nav-orders" role="tab" aria-controls="nav-orders" aria-selected="false">
